...
//    initialize heightmap and scoremap
    h_map = DMat<float>::Zero(x_size, y_size);//1030
    s_map = DMat<float>::Zero(x_size, y_size);//1120
    // init quadruped info
...
void Simulation::addCollisionBox(double mu, double resti, double depth,
                                 ...
                                 const Mat3<double>& ori, const Vec4<float>& rgb,
                                 bool addToWindow, bool transparent) {
    _simulator->addCollisionBox(mu, resti, depth, width, height, pos, ori);//添加碰撞
    if (addToWindow && _window) {
        _window->lockGfxMutex();
        _window->_drawList.addBox(depth, width, height, pos, ori, transparent,rgb);//画图
        _window->unlockGfxMutex();
    }
}
    ...
    if (buildTerrian){
        for (int i(0); i<h_map.rows();++i){
            for (int j(0); j <h_map.cols(); ++j) {//
                if (h_map(i,j)>0.02){ //仅对高于2cm的地形绘图
//                        std::cout<<_vel_cmd_pos[0]<<std::endl;
                    Vec3<double> pos;
                    pos[0] = ((h_map.rows()-i)*grid_map_size+0.2+_glo_robot_pos[0]);//x+camera_offset+global_robot_pos
                    pos[1] = ((h_map.cols()-j)*grid_map_size-1.0+_glo_robot_pos[1]);//y+map_offset+global_robot_pos
                    pos[2] = h_map(i,j)/2;// z  to do: +robot_z_pos

                    double pos_ori[3]={0.0,0.0,_glo_robot_pos[2]};//世界frame yaw
                    Mat3<double> pos_r = ori::rpyToRotMat(Vec3<double>(pos_ori));
                    pos=pos.transpose()*pos_r;
                    pos[0]=round(pos[0]*100)/100;
                    pos[1]=round(pos[1]*100)/100;
                    // 判断当前位置是否需要建图
                    if (int(pos[0]*1000)%20==0 && int(pos[1]*1000)%20==0) {
                        //绘制碰撞立方体地图+trav_score
                        double ori[3] = {.0, .0, 0.0};//立方体本身的姿态
                        Mat3<double> r = ori::rpyToRotMat(Vec3<double>(ori));
                        if (s_map(i, j) >= 0. && s_map(i, j) <= 0.1)
                            color = {0, 0.5, 1, 1.0};//blue
                        else if (s_map(i, j) > 0.1 && s_map(i, j) <= 0.8)
                            color = {0, 0.5, 1, 1.0};//blue
//                    box.color={0.5,1,0,1.0};//green
                        else if (s_map(i, j) > 0.8 && s_map(i, j) <= 3.)
                            color = {1, 1, 0, 1.0};//yellow
                        else if (s_map(i, j) > 3. && s_map(i, j) <= 5.0)
                            color = {1, 0.5, 0, 1.0};//orange
...
void Simulation::drawRealtimeTerrain(int fps) {
    ...
    if (fps%1==0){//0.25hz
            if (std::abs(pos_x-_glo_robot_pos[0])>0.015 ||std::abs(pos_y-_glo_robot_pos[1])>0.015
                ||std::abs(ori_yaw-_glo_robot_pos[2])>0.018){ //如果机器人位置变化才重新绘图

                for (int i(0); i<s_map.rows();++i){
                    for (int j(0); j <s_map.cols(); ++j) {//
                        if (s_map(i,j)>0.02){ //仅对高于2cm的地形绘图
//                        std::cout<<_vel_cmd_pos[0]<<std::endl;
                            Vec3<double> pos;
                            pos[0] = ((s_map.rows()-i)*grid_map_size+0.2+_glo_robot_pos[0]);//x+camera_offset+global_robot_pos
                            pos[1] = ((s_map.cols()-j)*grid_map_size-1.0+_glo_robot_pos[1]);//y+map_offset+global_robot_pos
                            pos[2] = s_map(i,j)/2;// z  to do: +robot_z_pos
                            ...
                            pos[0]=round(pos[0]*100)/100;
                            pos[1]=round(pos[1]*100)/100;
                            if (int(pos[0]*1000)%40==0 && int(pos[1]*1000)%40==0){
                            // 判断当前位置是否需要建图
//                            std::cout<<pos[0]<<" "<<pos[1]<<std::endl;
                                vector< double >::iterator result;
                                result=find(_x_log.begin(),_x_log.end(),pos[0]);
                                if (result!=_x_log.end()) {
                                    int index = std::distance(_x_log.begin(), result)+1;
                                    if (_y_log[index] == pos[1] && std::abs(_z_log[index] - s_map(i,j)) <= 0.02) {//若当前位置已有地图且高度值相差不超过2cm
                                        // do nothing
                                        drawmap=false;
                                    }
                                    else drawmap=true;//y无重合或z变化
                                }
                                else drawmap=true;//x无重合
                            }
                            else drawmap=false;
                            ...
                            if (drawmap){
                                ...
                                double ori[3]={.0,.0,0.0};//立方体本身的姿态
                                Mat3<double> r = ori::rpyToRotMat(Vec3<double>(ori));
                                addCollisionBox(0.7, 0, 0.041, 0.041, s_map(i,j), pos, r, color, true, false);
                                std::cout<<"x "<<pos[0]<<" y "<<pos[1]<<" block num: "<<_x_log.size()<<" h "<<s_map(i,j)<<std::endl;
//                                std::cout<<fin_pos[0]<<" "<<fin_pos[1]<<std::endl;
                                //更新地图数据
                                _x_log.push_back(pos[0]);
                                _y_log.push_back(pos[1]);
                                _z_log.push_back(s_map(i,j));
                            }
                        }
                    }
                }
                pos_x=_glo_robot_pos[0]; pos_y=_glo_robot_pos[1]; ori_yaw=_glo_robot_pos[2];
            }
        }
...
void Simulation::loadTerrainFile(const std::string& terrainFileName,
    ...
    for (auto &key : keys) {
        ...
        if (typeName == "infinite-plane") {
            ...
            // of "transformation"
            addCollisionBox(mu, resti, depth, width, height, Vec3<double>(pos), R_box,color,
            ...
            for (size_t step = 0; step < steps; step++) {
                ...
                p = R * p + pOff;
                addCollisionBox(mu, resti, run * (stepsDouble), width, rise + 0.01, p, R, color,
                                addGraphics, transparent != 0.);
...
//                    // 0928
//                    if (x_len == 333)
//                        heightmap333_lcm.map[i][j] = height_map_2d[i][j];
//                    //std::cout<<heightmap333_lcm.map[i][j]<<" ";
//                }
//                //std::cout<<std::endl;
//            }
...
//                             transparent != 0.);
    ...
    void Simulation::updateGraphics() {
        ...
        // publish loaded terrian heightmap data[333*333] 0928
//    bool a=false;//true;
//    if(a)
//        heightmap333LCM.publish("heightmap333", &heightmap333_lcm);
