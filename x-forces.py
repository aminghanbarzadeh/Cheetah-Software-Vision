...
# Now, we need to split the forces within each group (x, y, z) which are space-separated
# We'll use .apply() with a lambda function to split these columns further
...
# Plot X forces for all legs
plt.figure(figsize=(10, 6))
plt.plot(force_data['time'], force_data['leg_1_x_force'], label='Leg 1', color='blue')
plt.plot(force_data['time'], force_data['leg_2_x_force'], label='Leg 2', color='orange')
plt.plot(force_data['time'], force_data['leg_3_x_force'], label='Leg 3', color='green')
plt.plot(force_data['time'], force_data['leg_4_x_force'], label='Leg 4', color='red')
